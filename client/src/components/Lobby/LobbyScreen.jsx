...
const CARD_INFO = [
  {
    ...
    icon: "☣",
    color: "#ef5350",
    desc: "Infects humans. Zombies cannot hold Gun or Vaccine.",
  },
  {
    type: "vaccine",
    icon: "💉",
    color: "#26c6da",
    desc: "Cures a zombie. Single-use. No effect on humans.",
  },
  {
    type: "gun",
    icon: "🔫",
    color: "#ffa726",
    desc: "Kills any player instantly. Single-use.",
  },
  {
    type: "normal",
    icon: "🃏",
    color: "#78909c",
    desc: "Neutral card. No effect against anyone.",
  },
];
  ...
  return (
    <span
      style={{
        ...
        height: 8,
        borderRadius: "50%",
        background: colors[status] || "#546e7a",
        marginRight: 6,
        flexShrink: 0,
      }}
    />
...
export default function LobbyScreen() {
  ...
  const emitJoin = useCallback((socket, name, id) => {
    socket.emit(
      "joinLobby",
      { username: name, lobbyId: id || undefined },
      (res) => {
        setJoining(false);
  ...
  const handleCreate = useCallback(() => {
    ...
    if (socket.connected) {
      emitJoin(socket, name, null);
    } else {
      socket.once("connect", () => emitJoin(socket, name, null));
    }
  }, [username, emitJoin]);
  ...
  const handleJoinByCode = useCallback(() => {
    ...
    if (!code) {
      setError("Please paste the lobby code");
      return;
    }
    setError("");
    setJoining(true);
    const socket = connectSocket();
    if (socket.connected) {
      emitJoin(socket, name, code);
    } else {
      socket.once("connect", () => emitJoin(socket, name, code));
    }
        ...
        <div style={S.panel}>
          ...
          <div style={S.lobbyBox}>
            <div style={S.lobbyHeader}>
              ...
              <span style={{ color: "#263548", fontSize: 11 }}>
                {lobbyPlayers.length} / 20
              </span>
            </div>
            <div style={S.playerScroll}>
              {lobbyPlayers.length === 0 && (
                <div style={S.emptyList}>Waiting for players…</div>
              )}
              {lobbyPlayers.map((p) => (
          ...
          <div style={S.cardRow}>
            {CARD_INFO.map((c) => (
              <div
                key={c.type}
                style={{ ...S.cardChip, borderColor: c.color + "44" }}
              >
                <span style={{ fontSize: 16 }}>{c.icon}</span>
                <span
                  style={{ color: c.color, fontSize: 9, letterSpacing: 0.5 }}
                >
                  {c.type.toUpperCase()}
                </span>
              </div>
          ...
          <button
            style={{
              ...S.btnPrimary,
              opacity: lobbyPlayers.length < 2 ? 0.4 : 1,
            }}
            onClick={handleStart}
      ...
      <div style={S.panel}>
        ...
        <div style={{ marginBottom: 14 }}>
          <input
            ...
            onKeyDown={(e) =>
              e.key === "Enter" &&
              (mode === "join" ? handleJoinByCode() : handleCreate())
            }
            autoFocus
          />
        </div>
        ...
        {mode === "create" && (
          <div style={S.modePanel}>
            <p style={S.modeDesc}>
              Creates a private room. You'll receive a code to share with
              friends.
            </p>
            {error && <div style={S.errorBox}>{error}</div>}
            <button
        ...
        {mode === "join" && (
          <div style={S.modePanel}>
            ...
            <button
              style={{
                ...S.btnPrimary,
                opacity: joining || !lobbyCode.trim() ? 0.5 : 1,
              }}
        ...
        {/* How to play */}
        <div style={S.divider}>
          <span style={S.dividerText}>HOW TO PLAY</span>
        </div>
        <div style={S.rules}>
          {[
        ...
        <div style={S.divider}>
          <span style={S.dividerText}>CARDS</span>
        </div>
          ...
          {CARD_INFO.map((c) => (
            ...
            >
              ...
              <div>
                ...
                <span style={{ color: "#546e7a", fontSize: 11, marginLeft: 8 }}>
                  {c.desc}
                </span>
              </div>
            </div>
...
const S = {
  ...
  panel: {
    ...
    boxShadow: "0 0 80px rgba(41,182,246,0.06)",
  ...
  title: {
    ...
    lineHeight: 1,
    color: "#ef5350",
    textShadow: "0 0 24px rgba(239,83,80,0.5)",
  },
  gameName: {
    color: "#eceff1",
    fontSize: 22,
